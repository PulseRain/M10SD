...
//=======================================================================
// References:
//  [1] How to Use MMC/SDC, http://elm-chan.org/docs/mmc/mmc_e.html
//=======================================================================
...
//
// Parameters:
//      cmd             : 6 bit command for SPI mode
//      arg             : 32 bit argument specific to the cmd
//     response_type    : SD_CSR_R1_R1B / SD_CSR_R2 / SD_CSR_R3_R7
//     buf_top0_bottom1 : 0 - use byte 0-511 in the data buffer, 
//                        1 - use byte 512 - 1023 in the data buffer 
// 
// Return Value:
//      SD_RET_OK / SD_RET_TIME_OUT / SD_RET_CRC_FAIL
//
// Remarks:
//      Function to send command to Micro SD card in SPI mode
//----------------------------------------------------------------------------

static uint8_t SD_command(uint8_t cmd, uint32_t arg, uint8_t response_type, uint8_t buf_top0_bottom1)
{
  uint8_t t;
  ...
  t = buf_top0_bottom1 << 7;
  
  SD_CSR = SD_CSR_START | response_type | t;
...
//
// Parameters:
//      addr            : offset in the top/bottom half of the data buffer
//                        (0 - 511)
//
//     buf_top0_bottom1 : 0 - use byte 0-511 in the data buffer, 
//                        1 - use byte 512 - 1023 in the data buffer 
// 
// Return Value:
//      one byte data from the data buffer
//
// Remarks:
//      Function to read one byte data from the data buffer
//----------------------------------------------------------------------------

static uint8_t SD_read_one_byte (uint16_t addr, uint8_t buf_top0_bottom1)
{
  uint8_t t;
...
//
// Remarks:
//      disk_initialize() needed by Petit FatFs module
//----------------------------------------------------------------------------

static DWORD last_read_sector;
...
static DSTATUS disk_initialize ()
{
  ...
  last_read_sector = 0xFFFFFFFF;
  
  t = SD_command (0, 0, SD_CSR_R1_R1B, 0);

  if (t == SD_RET_TIME_OUT) {
      return RES_NOTRDY;
  }
  
  t = SD_read_one_byte(0, 0);

  if (t != SD_R1_IDLE_STATE) {
      return RES_NOTRDY;
  } 
  
  SD_CSR = SD_CSR_RESET | SD_SCLK_FAST;

  // From Ref[1]
  // After the card enters idle state with a CMD0, 
  // send a CMD8 with argument of 0x000001AA and correct CRC prior to initialization process
  
  SD_command(8, (uint32_t)0x000001AA, SD_CSR_R3_R7, 0);
  t = SD_read_one_byte (0, 0);
  
  if (t & SD_R1_ILLEGAL_CMD) {
    sd_card_type = SD_TYPE_SDC_V1;
  } else {
    sd_card_type = SD_TYPE_SDC_V2;
    
    t = SD_read_one_byte (4, 0);
     
    if (t != 0xAA) {
      return RES_NOTRDY; // CMD8 fail
    }
  }
  
  do {
    if (sd_card_type == SD_TYPE_SDC_V2) {
      SD_command(55, 0, SD_CSR_R1_R1B, 0);
      SD_command(41, (uint32_t)0x40000000, SD_CSR_R1_R1B, 0);  
    } else {
      SD_command(55, 0, SD_CSR_R1_R1B, 0);
      SD_command(41, 0, SD_CSR_R1_R1B, 0);  
    }
    t = SD_read_one_byte(0, 0);
  ...
  // When it is set, the card is a high-capacity card known as SDHC/SDXC
  
  if (sd_card_type == SD_TYPE_SDC_V2) {
    SD_command (58, 0, SD_CSR_R3_R7, 0);
    t = SD_read_one_byte(0, 0);
...
//
// Parameters:
//      buff    : [OUT] Pointer to the read buffer
//      sector  : [IN]  Sector number
//      offset  : [IN]  Byte offset in the sector to start to read
...
//
// Remarks:
//      disk_readp() needed by Petit FatFs module
//----------------------------------------------------------------------------

static DRESULT disk_readp (
...
)
{
  ...
  if (sector != last_read_sector) {
    t = SD_command(17, sector, SD_CSR_R1_R1B, 1);
    if (t != SD_RET_OK) {
      return RES_ERROR;
    }
...
//      sc      : [IN] Sector number or Number of bytes to write
//
// Return Value:
//      DRESULT
//
// Remarks:
//      disk_writep() needed by Petit FatFs module
//----------------------------------------------------------------------------

  
static DRESULT disk_writep (
  const BYTE* buff,  /* [IN] Pointer to the data to be written */
  DWORD sc    /* [IN] Sector number or Number of bytes to write */
) 
{
  ...
  static DWORD sector_to_write = 0xFFFFFFFF;
  static UINT buffer_address = 0;
  
  if (buff) {
    num_of_bytes = (UINT)sc;
    ...
    for (i = 0; i < num_of_bytes; ++i) {
        ...
        SD_BUF_ADDR = buffer_address & 0xFF;
        SD_DATA_OUT = *buff++;
        ++buffer_address;
    } // End of for loop
    ...
    t = SD_command(24, sector_to_write, SD_CSR_R1_R1B, 0);
...
static DWORD LD_DWORD(BYTE *ptr)
{
  WORD t;
  DWORD ret;
...
// Parameters:
//      ptr    : pointer to BYTE
//      val    : WORD value to be stored
//
// Return Value:
//      None
//
// Remarks:
//      store a WORD to the pointer
//----------------------------------------------------------------------------
...
// Parameters:
//      ptr    : pointer to BYTE
//      val    : DWORD value to be stored
//
// Return Value:
//      None
//
// Remarks:
//      store a DWORD to the pointer
//----------------------------------------------------------------------------
...
static int8_t mem_cmp (const uint8_t* dst, const uint8_t* src, uint16_t cnt) {
  const uint8_t *d = dst;
  const uint8_t *s = src;
...
static FRESULT pf_read (
  BYTE* buff,   /* Pointer to the read buffer (NULL:Forward data to the stream)*/
...
const M10SD_STRUCT SD = {sd_begin, pf_open, pf_read, pf_write};
